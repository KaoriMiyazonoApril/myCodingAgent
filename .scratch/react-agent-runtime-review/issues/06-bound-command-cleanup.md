...
- [x] 取消 `wait_task` 与 capture tasks 后不再通过无 timeout 的 `gather()` 等待其响应。
- [x] capture deadline 到达时主动关闭 stdout/stderr pipe transports，反复执行不会积累 FD。
- [x] cancellation-resistant task contract 测试证明清理 helper 仍在固定预算内返回。
