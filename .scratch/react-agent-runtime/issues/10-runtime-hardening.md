...
- [ ] 历史达到保守上下文预算时返回 `CONTEXT_LIMIT`，不静默删除或总结消息。
- [ ] Turn idempotency key 防止网络重试创建重复任务。
- [ ] Snapshot、Summary、事件和日志均不泄漏 API key、provider secret 或内部 traceback。
- [ ] 默认隐藏 reasoning，debug reasoning 仍遵守独立事件与内存保留规则。
- [ ] 规格列出的正常、失败、取消、并发、安全及多轮验收场景全部通过。
- [ ] 最终设计文档与实现保持同步。
