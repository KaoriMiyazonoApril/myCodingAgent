...
- [ ] 文件第一次修改前保存 original，后续多次修改最终只产生一份统一 diff。
- [ ] 新文件和最终文件状态在 Summary 与事件中正确表达。
- [ ] 已读取文件被外部改变后写入返回 `FILE_CHANGED`，模型可重新读取后重试。
- [ ] 文件工具修改保持 `diff_complete=true`。
- [ ] 可能通过 `run_command` 修改文件的 Turn 明确报告 `diff_complete=false`。
