...
- Filesystem tests use temporary workspaces and cover normal read, bounded chunk pagination across UTF-8 and CRLF boundaries, missing files; write create, overwrite and parent creation; edit success, zero-match immutability, multiple-match immutability, global replacement, and empty old-string rejection.
