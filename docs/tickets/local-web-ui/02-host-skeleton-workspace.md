...
- [x] WSL 路径原样显示为 Host POSIX path，不在浏览器中尝试 Windows/WSL 转换。
- [x] Workspace picker 授权与 Runtime workspace validation 保持独立，UI 不把可浏览等同于可执行。
- [x] 开发 CORS 只允许约定的 Vite origin；生产 same-origin 设计不引入宽泛 CORS。
- [x] Host application tests 覆盖多个 root、排序、隐藏目录、截断、symlink、权限和 escape；React tests 覆盖导航与错误。
