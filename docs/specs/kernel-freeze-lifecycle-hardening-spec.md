...
- Cancelling a cross-Turn poll unwinds only that interaction; it does not close the older owner Turn's Session. Turn cancellation also performs owner-filtered cleanup at the Turn boundary, so Sessions returned as `running` before a later model call are still reaped.
...
- Each pooled adapter holds a Turn-scoped lease. Capacity eviction may close only an idle transport; if every bounded slot is in flight, resolution fails closed instead of interrupting an active request. Turn success, failure, cancellation and preflight failure release the adapter in `finally` paths.
