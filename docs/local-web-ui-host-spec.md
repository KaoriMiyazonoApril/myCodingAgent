...
- The largest Host-specific correctness risk is the interval before `turn_started`: synchronous workspace validation currently delays the first await, and failures currently have no terminal event. Asynchronous validation plus `turn_rejected` is a narrow Runtime enhancement that preserves the AgentLoop seam while making background submission observable.
