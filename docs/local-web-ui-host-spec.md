...
- Conversation renders user and assistant messages, tool request/running/result cards, bounded approval cards recovered from Snapshot/SSE, file-change cards, Turn errors, Runtime rejection, and Host connection/configuration errors. When a host-provided reasoning preview or delta exists (`visible`/`debug` hosts), assistant messages show the thinking chain in a collapsible panel (open while streaming, collapsed afterwards, marked when truncated); on the default `hidden` Web Host no raw reasoning is rendered. Rejection errors display the `detail` and a recovery hint for transient busy codes.
