...
- Reasoning never enters canonical Conversation messages or Snapshot serialization. The Web Host runs Threads at the runtime default `reasoning_visibility="hidden"`: normal Web clients receive no `model_reasoning_delta` stream and no durable `reasoning_preview`, so a long thinking phase is not rendered as raw chain text. Hosts/embedders may opt in to `visible` (live deltas plus a bounded `reasoning_preview {text, truncated, total_chars}` on the durable `model_response` event) or `debug` (complete chain via the durable `model_reasoning` event). Hiding the public UI stream never strips `ReasoningBlock`s from the canonical Conversation, so multi-turn provider retention keeps working.
- Instead of raw reasoning, every model call emits the transient `model_activity` event **only when the activity phase changes**: `thinking` on the first reasoning delta, `writing` on the first text delta, `acting` on the first tool-call delta, and `idle` when the response ends (the timer anchor is the announcing event's timestamp; payload carries only `{phase}`, never text and never per-token). The UI starts a local count-up on `thinking` ("思考中… 12.3s") and freezes the duration on a phase switch or `idle` ("思考 · 18.7s"). `model_activity` is transient: it is never mirrored to the durable sink.
